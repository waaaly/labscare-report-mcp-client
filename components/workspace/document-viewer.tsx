...
import { useDocUploadProgress } from '@/hooks/useDocUploadProgress';
...
const DocumentViewer = forwardRef<DocumentViewerHandle, DocumentViewerProps>(
  ({ projectId, selectedDocumentId }, ref) => {
    ...
    const uploadProgress = useDocUploadProgress(selectedDocumentId || '');
        ...
        const result =
          await callMcpTool(
            'parse_labscare_docx',
            {
              ...
              table_desc: documentDescription
            },
            {
              timeout: 120_000,
              onLog: (level, message) => {
    ...
    const renderPreview = () => {
      ...
      // 检查文档处理状态
      if (selectedDocument.status && selectedDocument.status !== 'COMPLETED') {
        return (
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-24">
              <Loader2 className="h-16 w-16 text-cyan-500 animate-spin mb-4" />
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Processing {selectedDocument.name} Document</h3>
              <p className="text-gray-600 text-center max-w-md mb-4">
                {selectedDocument.status === 'PENDING' && 'Waiting for processing...'}
                {selectedDocument.status === 'PROCESSING' && 'Document is being processed...'}
                {selectedDocument.status === 'FAILED' && 'Processing failed. Please try again.'}
                {uploadProgress.msg && <span className="block mt-2">{uploadProgress.msg}</span>}
              </p>
              {selectedDocument.status === 'PROCESSING' && (
                <div className="w-full max-w-md mb-4">
                  <div className="flex  -between text-sm text-gray-600 mb-1">
                    <span>Progress</span>
              ...
              {selectedDocument.status === 'FAILED' && (
                <Button onClick={handleProcessDocument} className="mt-4">
                  Retry Processing
                </Button>
              )}
            </CardContent>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="space-y-2">
                  <label htmlFor="document-description" className="text-sm font-medium text-gray-700">
                  ...
                  <textarea
                    ...
                    placeholder="Enter a description of the document structure..."
